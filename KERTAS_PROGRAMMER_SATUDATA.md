...
| 6 | 2026-05-19 | 2026-05-19 | Setup lingkungan Production & deploy tooling | Pisah SSH dev (`ssh-banten-dev`) vs prod (`ssh-banten-prod` → 10.249.100.25). Tambah `deploy/environments.md`, `deploy/sync.sh`, `deploy/ssh-config.snippet`. Update `.env.production.example` domain prod. Deploy manual prod confirmed work. Detail di `tasks/2026-05-19-22-production-go-live.md` Card 1. | Done |
| 7 | 2026-05-19 | 2026-05-19 | Release notes & API flow go-live | `docs/RELEASE-NOTES.md` (konsolidasi 3 portal → satu domain), `docs/api-flow.md` + diagram via API Manager (`api.bantenprov.go.id`). Detail Card 6. | Done |
| 8 | 2026-05-20 | 2026-05-20 | Update `CKAN_SITE_URL` production | `.env` prod: `satudata.bantendev.id` → `https://data.bantenprov.go.id`, recreate container CKAN. Before/after: link & cookie root URL prod. Detail Card 2. | Done |
| 9 | 2026-05-20 | 2026-05-20 | Verifikasi SMTP production | Samakan `CKAN_SMTP_*` prod dengan dev; kirim email uji ke hery.handoko@gmail.com — terkirim OK. Detail Card 3. | Done |
| 10 | 2026-05-20 | 2026-05-20 | Fix URL API hardcode di template | Helper `banten_site_url()` / `banten_api_action_url()`; homepage + `/dokumentasi` tidak lagi pakai `satudata.bantendev.id`. Deploy prod verified. Detail Card 4. | Done |
| 11 | 2026-05-21 | 2026-05-22 | Metrix Metabase embed production | `metabase_embed.py`, nginx config referensi, workaround `METABASE_DIRECT_DASHBOARD_URL`. Before: /metrix prod broken (404 JS). After: work **dengan VPN**; publik tanpa VPN masih open (butuh nginx + MB_SITE_URL). Detail Card 5. | Partial |
